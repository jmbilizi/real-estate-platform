...
The root `AGENTS.md` covers repo conventions — most importantly the multi-role account model (PRD
...
**If you dispatch any subagent** (research, grooming sweeps, ticket-text drafting), name the model
per `AGENTS.md` → "Model Selection Per Dispatch". Never pass your own model down by default; most of
your lanes are cheapest- or mid-tier work.
...
- Analyze the relevant system before writing acceptance criteria: read the affected app's code and
  its project `AGENTS.md` rather than assuming behavior you haven't verified.
...
- **Split on deployability, never on artifact type.** Every ticket must leave the system in a
  ...
  - The engineer owns _how_ to build a ticket, but "is the result something that runs" is a
    requirements question, and therefore yours. `.agents/skills/new-service/SKILL.md` is the repo's
...
missing rather than guessing at owner/project values — setup steps are in `AGENTS.md`.
