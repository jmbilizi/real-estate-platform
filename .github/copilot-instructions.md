...
marker — one quoted in backticks or a fence is prose and is fine — is rejected outright), while
`update-status --plan-file` rewrites only the plan block and cannot touch the product owner's
sections. Unit tests for both directions live in `tools/github/lib/issue-body.test.js` — run them
with `pnpm run tools:test`. Unknown labels are validated by `gh` at write time rather than
pre-checked locally, so a rejected label can leave earlier edits in the same invocation already
applied — e.g. `--issue 42 --status Ready --add-label typo` writes Status first, then fails on the
label, and the Status change stays. That's the accepted trade-off of letting `gh` reject unknown
labels, not a bug; a failed call isn't necessarily an atomic no-op.
