 ...
 *
 *  - reports `not_configured` and finishes **successfully** — the expected state while an
 *    environment waits for #117 to provision its secret; or
...
function notConfiguredMessage(config: Extract<BrightConfig, { state: 'not-configured' }>): string {
  return (
    ...
    'the committed placeholder. Nothing was ingested and nothing was written. It means #117 has ' +
    "not yet provisioned this environment's secret. See " +
