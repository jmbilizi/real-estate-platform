 ...
 *
 * This module enforces the ENDPOINT half of that invariant and not the credential half, and the gap
 * is worth being explicit about. `resolveBrightConfig` checks that a credential is present, that an
 * endpoint is HTTPS, and — new in #92 — that a `test` tier declaration is not pointed at a
 * production host. It does not check that the credential belongs to the feed the endpoint names,
 * because a client id carries no evidence of which tier issued it. #164 is where that check lands.
 *
 * Bright itself closes most of that gap: measured 2026-09-19, a test credential presented to the
 * production token endpoint is refused with HTTP 400, because the two tiers are separate Okta
 * tenants. So a misfiled credential is rejected rather than silently working.
 *
