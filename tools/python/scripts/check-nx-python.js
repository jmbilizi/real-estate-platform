...
function isNxPythonSetup() {
  ...
  const nxPythonConfigExists = fs.existsSync(
    path.join(rootDir, "nx-python.json"),
  );

  // Check 2: Is @nxlv/python installed in node_modules?
  const nxPythonModuleExists = fs.existsSync(
    path.join(rootDir, "node_modules", "@nxlv", "python"),
  );
  ...
  const checksPassedCount = [
    nxPythonConfigExists,
    nxPythonModuleExists,
    nxJsonHasPython,
...
if (require.main === module) {
  ...
  console.log(
    isSetup ? "NX Python is already set up." : "NX Python is not set up yet.",
  );
