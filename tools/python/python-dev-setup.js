...
function installGlobalTools() {
  ...
  // Update/create .env file with UV and Poetry paths (preserve existing content)
  ...
  try {
    ...
    // Merge new variables (preserve others like MAXMIND_LICENSE_KEY)
    const mergedVars = { ...existingVars, ...newVars };

    // Generate new content
    const lines = ["# Auto-generated by python-dev-setup.js", "# UV and Poetry paths for immediate use in npm scripts"];

    // Add Python-related variables first
    ["UV_PATH", "POETRY_PATH", "PIPX_BIN_PATH"].forEach((key) => {
      if (mergedVars[key]) {
    ...
    // Add any other existing variables (e.g., MAXMIND_LICENSE_KEY)
    ...
    log("Updated .env file with UV and Poetry paths (preserved existing variables).");
  } catch (error) {
    log(`Warning: Could not update .env file: ${error.message}`, true);
  }
