...
function addLocalRegistrySafetyFlags(argsList) {
  ...
  const defaultRepoIndex = argsList.findIndex((a) => a === "--default-repo" || a.startsWith("--default-repo="));
...
function getLocalKubeContextCandidates() {
  const configPath = path.resolve(workspaceRoot, "infra/k8s/podman/local/cluster/cluster-config.yaml");
  ...
  const candidates = [];
  if (config.kubectl_context && typeof config.kubectl_context === "string" && config.kubectl_context.trim()) {
...
function ensureLocalKubeContext() {
  ...
  if (switched.status !== 0) {
    console.error(`ERROR: Refusing to run Skaffold without the intended local kube-context (${preferred}).`);
    process.exit(1);
  }
}
