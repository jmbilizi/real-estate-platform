...
function isLocalDevRegistry(registryHost) {
  return (
    registryHost === `localhost:${REGISTRY_PORT}` || registryHost === `127.0.0.1:${REGISTRY_PORT}`
  );
}
...
function writeRetentionState(state) {
  ...
  fs.writeFileSync(
    tmpFile,
    JSON.stringify({ version: 1, repos: state.repos || {} }, null, 2) + '\n',
    'utf8',
  );
...
function garbageCollectRegistry() {
  ...
  if (res.status !== 0) {
    ...
    console.warn(
      'WARN: registry garbage-collect failed; consider resetting local registry volume if disk grows.',
    );
  }
}
