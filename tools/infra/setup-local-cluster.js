...
function loadClusterConfig() {
  ...
  assertField(
    raw.cluster_name && typeof raw.cluster_name === 'string',
    'cluster_name must be defined',
  );
  assertField(
    raw.nodes && Number.isInteger(raw.nodes) && raw.nodes > 0,
  ...
  assertField(
    raw.resources.memory && raw.resources.memory > 0,
    'resources.memory must be > 0 (MB)',
  );
  assertField(
  ...
  return {
    ...
    kubernetesVersion: raw.kubernetes_version.startsWith('v')
      ? raw.kubernetes_version
      : `v${raw.kubernetes_version}`,
  };
}
...
function ensurePodmanMachine() {
  ...
  if (machines.length === 0) {
    ...
    if (!initResult.success) {
      logError(
        "Failed to initialize Podman machine. Please run 'podman machine init' manually and retry.",
      );
      process.exit(1);
    }
...
function installCABundleInPodmanMachine(machineName) {
  ...
  const remoteResult = run(`podman machine ssh ${machineName} "${escapedScript}"`, {
    silent: true,
  });
  ...
  if (installResult.success) {
    ...
    logWarning(
      'Unable to install enterprise CA bundle automatically. Please add your corporate CA manually.',
    );
  }
}
...
function exportHostRootCertificates() {
  switch (process.platform) {
    ...
    default:
      logWarning(
        `Host trust export not implemented for platform: ${process.platform}. Skipping corporate CA sync.`,
      );
      return null;
  }
}
...
function saveDockerBuildCABundle(caBundle) {
  ...
  try {
    ...
    logSuccess(
      `Saved CA bundle for Docker builds: ${path.relative(WORKSPACE_ROOT, dockerCertPath)}`,
    );
  } catch (error) {
...
function listKindNodes() {
  const result = run(
    `podman ps --format {{.Names}} --filter label=io.x-k8s.kind.cluster=${KIND_CLUSTER_NAME}`,
    {
      silent: true,
    },
  );
...
function configureNodeSystemTrust(nodeName, caBuffer) {
  ...
  if (!updateResult.success) {
    logWarning(
      `update-ca-certificates failed on ${nodeName}. TLS for OS utilities may still break.`,
    );
...
// even sent, which no CA trust fix can address. A raw query for the same hostname against a public
// resolver, bypassing the search list, returns only the legitimate address and lets the handshake
// proceed.
//
// The fix removes the enterprise search domains from the node's resolv.conf (kept only there — a
// pod's own resolv.conf is generated by kubelet per pod, not edited directly) and points at public
// resolvers. Two of them, so neither being unreachable on a given network blocks image pulls or
// app traffic. A workstation with no enterprise search domains has nothing to remove.
const PUBLIC_DNS_SERVERS = ['1.1.1.1', '8.8.8.8'];

function buildResolvConf(nameservers) {
  return [...nameservers.map((ip) => `nameserver ${ip}`), ''].join('\n');
}

function configureNodePublicDns(nodeName) {
  const resolvConf = buildResolvConf(PUBLIC_DNS_SERVERS);
...
// default. Rewriting it here therefore changes external DNS for every pod in the cluster, with no
// CoreDNS or kubelet configuration to touch. A workstation with no corporate DNS override sees the
// same public answer a normal ISP resolver would give it, so this is a true no-op there.
function propagateDnsFixToKindNodes() {
  ...
  nodes.forEach((node) => {
    if (configureNodePublicDns(node)) {
      logSuccess(`Pointed ${node} at public DNS (${PUBLIC_DNS_SERVERS.join(', ')})`);
    } else {
      logWarning(`Could not point ${node} at public DNS; external TLS may still fail (#180).`);
    }
  });
}
...
function createKindCluster() {
  ...
  logInfo(
    `Creating Kind cluster '${KIND_CLUSTER_NAME}' using kindest/node:${CLUSTER_CONFIG.kubernetesVersion}`,
  );
  const command = [
    'kind create cluster',
...
function ensureKubectlContext() {
  ...
  if (result.success) {
    ...
    logWarning(
      `Could not switch kubectl context to ${KIND_CONTEXT}. You may need to run it manually.`,
    );
  }
}
...
function applyLocalResources() {
  ...
  const result = run(
    'kustomize build infra/k8s/podman/local --enable-alpha-plugins | kubectl apply -f -',
  );
  if (result.success) {
    logSuccess('Local resources applied successfully');
  } else {
    logWarning(
      'Failed to apply resources automatically. Run pnpm run infra:local:k8s-resources:apply later.',
    );
  }
}
