...
function binaryAvailable(cmd) {
  const probeArgs = process.platform === "win32" ? ["/c", "where", cmd] : ["-lc", `command -v ${cmd}`];
...
if (!runtime) {
  console.error("ERROR: Neither podman nor docker was found in PATH. Need one to exec into Kind nodes.");
  process.exit(1);
}
